...
let models = []; // 3Dモデルの配列
let textures = []; // テクスチャの配列
let selectedIndex = -1; // 選択されたアイテムのインデックス


function preload() {
  // モデルとテクスチャをロード
  const modelPaths = [
    'assets/models/fireman/Fireman_0122044847_texture.obj'
  ];
  const texturePaths = [
    'assets/textures/fireman/Fireman_0122044847_texture.png'
  ];

  for (let i = 0; i < modelPaths.length; i++) {
    models.push(loadModel(modelPaths[i], true));
    textures.push(loadImage(texturePaths[i]));
  }

  if (models.length > 0 && textures.length > 0) {
    console.log("3Dモデルとテクスチャがロードされています");
  } else {
    console.log("モデルまたはテクスチャがまだロードされていません");
  }
}
